...
const CustomGeometry: FC = () => {
  const { coorinatesMatrix, indicesMatrix, colors, displacement, display, displacementScale } = useAppSelector(
    (store) => store.model,
    shallowEqual
  )

  const { displayNodeIndices, displayLight } = useAppSelector((store) => store.modelViewSetting, shallowEqual)
